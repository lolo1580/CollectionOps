...
| Objet | Identifiant stable, espace courant, nom non vide, description et références historique/technique facultatives, numéro d'inventaire attribué par le serveur, révision positive, état `active` / `archived` / `trashed`. | Appartient à exactement un espace courant. |
| Série ou regroupement | Identifiant, espace, type `series` / `group`, nom, révision. | Un objet peut appartenir à plusieurs ensembles de son espace courant ; seule une définition vide peut être supprimée. |
...
8. Un objet est `active`, `archived` ou `trashed` (D05). Chaque transition vérifie la révision attendue, incrémente la révision et ajoute un événement d'audit avec l'auteur et l'état avant/après. Aucune suppression physique n'existe en V1 : l'identifiant, le numéro et l'historique sont conservés quel que soit l'état, et les numéros ne sont jamais réutilisés. Un objet `trashed` est en lecture seule jusqu'à sa restauration.
...
La [migration de révision des acquisitions](../../backend/migrations/202609240001_acquisition_revisions.sql) ajoute une révision initiale `1` aux trois entités existantes. Chaque mise à jour compare la révision attendue dans la clause `UPDATE` avant de l'incrémenter ; les écritures concurrentes ne se remplacent donc pas silencieusement.

La [migration des demandes de création d'objet](../../backend/migrations/202609240003_item_creation_requests.sql) conserve, par espace et compte, une clé UUID et la réponse initiale. Elle permet de rejouer une création sans réattribuer un numéro ; elle ne couvre pas encore les autres commandes hors ligne.

La [migration documentaire](../../backend/migrations/202609240004_item_documents.sql) conserve les métadonnées des fichiers (objet, espace propriétaire, nom, type MIME, taille, SHA-256, auteur, date et retrait logique). Le fichier lui-même réside dans un répertoire privé configuré hors de MariaDB, sous un nom UUID généré par le serveur. L'espace du document ne change pas lors d'un transfert de l'objet : les anciens membres autorisés peuvent encore lire ce document dans la source, sans le révéler à la destination. Le verrou de l'objet sérialise les ajouts avec les transferts et avec le contrôle de quota. Les fichiers retirés logiquement restent sur disque en attendant la politique de rétention ; la sauvegarde doit couvrir simultanément la base et le répertoire.
...
Lors d'un remplacement de classement dans le même espace, les affectations conservées ne changent pas. Les affectations retirées reçoivent `ended_at`, les nouvelles sont insérées. Une valeur d'un champ encore disponible via la nouvelle sélection est copiée sur une affectation active si aucune valeur active n'existe déjà ; les valeurs devenues inapplicables restent attachées aux affectations terminées, sans être affichées. Le tout est atomique et protégé par la révision de l'objet. Une sélection inchangée n'incrémente pas cette révision.

Les emplacements ont un parent facultatif dans le même espace et un nom unique parmi leurs frères. `inventory_items.current_location_id` est facultatif ; une clé étrangère composite avec `space_id` empêche d'y enregistrer un emplacement d'un autre espace. Chaque changement réel crée un événement avec auteur, ancienne et nouvelle position dans la même transaction que l'incrément de révision. Un transfert d'espace inscrit une sortie vers `null` dans l'espace source, puis vide l'emplacement courant avant le changement d'espace. L'API n'expose que le journal de l'espace courant, sans révéler les mouvements d'anciens espaces.
