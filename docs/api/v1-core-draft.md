...
Chaque envie, vendeur et offre expose désormais une `revision` décimale en chaîne. `PATCH /api/v1/spaces/{space_id}/wishes/{wish_id}` reçoit `title`, `search_notes` et `expected_revision` ; `PATCH /api/v1/spaces/{space_id}/vendors/{vendor_id}` reçoit `name`, `website_url` et `expected_revision` ; `PATCH /api/v1/spaces/{space_id}/wishes/{wish_id}/offers/{offer_id}` reçoit `vendor_id`, `title`, `source_url`, `notes` et `expected_revision`. Les champs facultatifs peuvent être `null` pour les vider. Chaque mise à jour autorisée incrémente la révision, même si le contenu est identique. Une révision obsolète reçoit `409` sans écraser l'état courant ; un identifiant d'un autre espace reçoit `404`. Les mises à jour exigent `acquisitions_write` et restent sans montant financier.

`GET /api/v1/spaces/{space_id}/items` retourne `{"items":[...],"next_cursor":null}` trié par numéro d'inventaire. L'accès requiert une session, une adhésion et `collections_read`. Un membre sans droit reçoit `403`, un non-membre reçoit `404`. La taille de page `limit` vaut 50 par défaut et doit rester entre 1 et 100. `after` est le dernier numéro d'inventaire de la page précédente ; `next_cursor` contient ce numéro comme chaîne quand une autre page existe. `q` cherche une sous-chaîne dans le nom, la description et les références historique et technique ; `%` et `_` sont des caractères ordinaires, pas des jokers. Une recherche vide liste tous les objets. `state` filtre selon `active` (défaut), `archived`, `trashed` ou `all`. `category_id` filtre les objets classés dans la catégorie choisie ou une de ses sous-catégories actives ; `location_id` filtre l'emplacement courant et ses descendants ; `group_id` filtre les objets membres d'une série ou d'un regroupement. Un identifiant d'un autre espace reçoit `404`, un identifiant malformé `422`. Les filtres se combinent à la recherche, à l'état et au curseur. Un curseur invalide, une limite hors plage, un état inconnu ou une recherche de plus de 100 caractères reçoit `422`.

Pour `POST /api/v1/spaces/{space_id}/items`, le client peut envoyer `Idempotency-Key: <UUID>` ; il doit réutiliser la **même clé** lorsqu'il réessaie la même création après une réponse perdue. Le serveur mémorise dans la même transaction la clé, le nom normalisé et la réponse initiale, par compte et par espace. Une répétition du même nom et de la même clé retourne le même objet avec `201`, sans consommer un second numéro, même si l'objet a été modifié depuis. La même clé avec un autre nom reçoit `409` (`idempotency_key_reused`) ; une clé malformée reçoit `422` (`invalid_idempotency_key`). Une nouvelle clé représente une nouvelle création, même si le nom est identique. Sans en-tête, le comportement historique est conservé. Les clés sont conservées sans expiration pour cette première tranche ; leur rétention sera réexaminée avec le protocole de synchronisation.
...
## Documents et photographies — première tranche backend

Le backend expose `GET` et `POST /api/v1/spaces/{space_id}/items/{item_id}/documents`, puis `GET` et `DELETE /api/v1/spaces/{space_id}/items/{item_id}/documents/{document_id}`. `GET` sur la liste renvoie les métadonnées actives ; `GET` sur un identifiant renvoie les octets avec leur type MIME et `Content-Disposition: attachment`. `POST` envoie directement les octets avec `Content-Type: image/jpeg`, `image/png`, `image/webp` ou `application/pdf`, et `x-document-name` contenant le nom UTF-8 encodé en pourcentage. Le backend vérifie aussi la signature du contenu. La réponse `201` expose l'identifiant, le nom, le type, la taille, le SHA-256, l'auteur et la date.

La lecture exige `documents_read` ; l'ajout et le retrait logique exigent `documents_write` dans l'espace indiqué. Aucun droit n'est déduit de `collections_read`. Les routes sont indisponibles (`503`) si `COLLECTIONOPS_DOCUMENTS_DIR` n'est pas configuré. La limite est de 50 Mo (50 000 000 octets) par fichier et de 50 documents actifs par objet **dans chaque espace**. `DELETE` masque le document mais conserve son fichier jusqu'à définition d'une politique de purge. Après un transfert, les documents restent consultables par les membres autorisés de l'espace source via son `space_id`, et ne sont pas visibles dans l'espace de destination. Un ajout dans l'ancien espace est refusé (`404`).

## Hors de cette première tranche

La liste des sessions et la connexion sont exposées depuis le 2026-09-22 (voir [ADR-0003](../architecture/ADR-0003-local-credentials-and-sessions.md)). Les invitations à droits sélectionnables, la gestion des membres, le renommage simple, l'archivage et la corbeille sont exposés depuis le 2026-09-23 ; les montants, l'import/export, S3 et la synchronisation exigent encore leurs contrats spécifiques. Seule la création d'objet accepte à ce stade une clé de rejeu ; l'idempotence des autres commandes reste à définir avant ouverture aux modifications hors ligne.
