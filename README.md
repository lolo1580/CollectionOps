...
Ouvrir `client-windows/CollectionOps.Client/CollectionOps.Client.csproj` dans Visual Studio, sélectionner `x64` ou `ARM64`, puis lancer le projet. Dans **Paramètres**, saisir l'adresse du serveur et choisir le thème. Dans **Compte**, se connecter ou accepter une invitation. Le client sépare **Inventaire**, **Acquisitions**, **Documents**, **Organisation** et **Partage** dans la navigation. L'inventaire propose la recherche et la liste à gauche, la fiche à droite ; on peut créer un espace ou un objet, filtrer, modifier la fiche, renommer les catégories et les champs, relier l'objet à d'autres objets, archiver, restaurer, transférer et consulter l'historique selon les droits. Dans **Documents**, sélectionner un objet puis ouvrir « Photos et documents » pour ajouter, consulter, enregistrer/ouvrir ou retirer un fichier selon les droits du membre ; le serveur doit disposer de `COLLECTIONOPS_DOCUMENTS_DIR`. Le **Partage** permet d'inviter, de gérer les membres et, pour le propriétaire actuel, de transférer la propriété à un membre existant. **Finances** reste un aperçu sans persistance. Un administrateur qui n'est pas membre peut administrer les droits sans consulter l'inventaire. Le backend nécessite `COLLECTIONOPS_DATABASE_URL`. Le jeton, l'adresse du serveur, l'option HTTP LAN et le thème restent uniquement en mémoire jusqu'à la fermeture du client. HTTPS est requis à distance ; HTTP est accepté pour la boucle locale, ou pour une adresse IPv4 privée seulement après activation explicite dans **Paramètres**. Cette exception de développement transmet les identifiants et jetons sans chiffrement sur le LAN ; elle ne doit pas servir en production. Le mode hors ligne n'est pas encore disponible.
...
| `COLLECTIONOPS_DOCUMENTS_DIR` | Répertoire absolu privé pour les fichiers ; active les routes documentaires | aucune |
...
Le stockage documentaire local est facultatif et désactivé sans `COLLECTIONOPS_DOCUMENTS_DIR`. Le répertoire doit être absolu ; sous Linux, il doit être privé (`0700`) et les fichiers sont créés en `0600`. Les métadonnées résident en MariaDB, mais **la sauvegarde/restauration doit couvrir la base et ce répertoire ensemble**. La migration accorde explicitement `documents_read` et `documents_write` aux propriétaires des espaces déjà créés ; un nouvel espace reçoit les mêmes droits documentaires, sans droit financier implicite. Les fichiers retirés logiquement restent sur disque jusqu'à validation d'une politique de purge. Le stockage S3, la déduplication et l'analyse antivirus ne font pas encore partie de cette tranche.

Pour le service systemd de développement utilisant `DynamicUser=yes`, ajouter `StateDirectory=collectionops` et `StateDirectoryMode=0700` dans sa section `[Service]`, puis définir `COLLECTIONOPS_DOCUMENTS_DIR=/var/lib/collectionops/documents` dans `/etc/collectionops/dev.env`. Sauvegarder la base **et** ce répertoire avant toute migration ; ne pas activer cette option sur le LXC tant que la nouvelle suite `documents_http` n'a pas passé sur `collectionops_test`.
